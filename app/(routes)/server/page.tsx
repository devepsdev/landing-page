...
const formatPorts = (
  ...
  return (
    Object.entries(ports)
      .map(([proto, bindings]) =>
        bindings?.length ? `${bindings[0]}→${proto}` : proto,
      )
      .join(", ") || "—"
  );
};
...
const Card = ({
  ...
  <div className={`p-4 rounded-lg bg-darkBg ${className}`}>
    <h2 className="mb-3 text-xs font-bold tracking-widest uppercase text-secondary">
      {title}
    </h2>
    {children}
  </div>
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
              ...
              <div className="grid grid-cols-2 gap-x-4 gap-y-2 text-sm">
                ...
                <div>
                  ...
                  <p className="font-semibold text-xs">
                    {formatLoadAvg(data.cpu.load_avg)}
                  </p>
                </div>
              </div>
            </Card>
            ...
            <Card title="Memoria">
              <div className="mb-3">
                <div className="flex justify-between mb-1 text-sm">
                  ...
                  <span className="font-semibold">
                    {(data.memory.ram_used_mb / 1024).toFixed(1)} /{" "}
                    {(data.memory.ram_total_mb / 1024).toFixed(1)} GB
                  </span>
                </div>
              ...
              {data.memory.swap_total_mb > 0 && (
                <div>
                  <div className="flex justify-between mb-1 text-sm">
                    ...
                    <span className="font-semibold">
                      {(data.memory.swap_used_mb / 1024).toFixed(1)} /{" "}
                      {(data.memory.swap_total_mb / 1024).toFixed(1)} GB
                    </span>
                  </div>
                  <ProgressBar
                    pct={
                      (data.memory.swap_used_mb / data.memory.swap_total_mb) *
                      100
                    }
                  />
                </div>
            ...
            <Card title="Certificado SSL">
              ...
              <div className="flex items-center gap-2 mb-1">
                ...
                <span
                  className={`text-sm font-semibold ${sslColor(data.ssl.days_remaining)}`}
                >
                  {data.ssl.days_remaining} días restantes
                </span>
              </div>
              <p className="text-xs text-gray-400">
                Caduca: {data.ssl.expiry_date}
              </p>
            ...
            <Card title="Sistema">
              <div className="grid grid-cols-2 gap-x-4 gap-y-2 text-sm">
                ...
                <div>
                  ...
                  <p
                    className={`font-semibold text-xs ${data.system.pending_updates > 0 ? "text-yellow-400" : "text-green-400"}`}
                  >
                    {data.system.pending_updates > 0
                      ? `${data.system.pending_updates} pendientes`
                      : "Al día"}
                  </p>
                </div>
              </div>
                    ...
                    <div key={disk.mountpoint}>
                      <div className="flex justify-between mb-1 text-sm">
                        ...
                        <span className="font-semibold text-xs">
                          {disk.used_gb.toFixed(1)} / {disk.total_gb.toFixed(1)}{" "}
                          GB
                        </span>
                      </div>
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="text-left text-gray-400 border-b border-white/10">
                        ...
                        <th className="pb-2 font-normal pr-4 hidden md:table-cell">
                          Imagen
                        </th>
                        <th className="pb-2 font-normal pr-4 hidden md:table-cell">
                          Puertos
                    ...
                    <tbody>
                      {Array.from(data.docker).map((c) => (
                        ...
                        >
                          ...
                          <td className="py-2 pr-4 text-xs text-gray-400 hidden md:table-cell">
                            {c.image}
                          </td>
                          <td className="py-2 pr-4 text-xs text-gray-400 hidden md:table-cell">
                            {formatPorts(c.ports)}
                          </td>
                          <td className="py-2 text-xs text-gray-400 hidden md:table-cell">
                            {c.uptime ?? "—"}
                          </td>
                        </tr>
                      ))}
                  ...
                  {Array.from(data.services).map((svc) => (
                    ...
                    >
                      ...
                      <span className="text-xs text-gray-400 shrink-0">
                        {svc.memory_mb != null ? svc.memory_mb.toFixed(0) : "—"}{" "}
                        MB
                      </span>
                    </div>
                  ))}
